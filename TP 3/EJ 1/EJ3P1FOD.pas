...
procedure menu (var arch: arch_emp; var nomF: strings; var creado: boolean);
	...
	procedure eliminacion (var arch: arch_emp; nomF: strings; creado: boolean);
	var
		pos: integer;
		e: empleados;
	begin	
		reset(arch);
		writeln('Ingrese la posicion del archivo que desea borrar: ');
		readln(pos);
		if (pos >= 0) and (pos <= fileSize(arch)) then begin
...
begin
	...
	writeln ('J. Baja logica de uno de los empleados truncando la ultima posicion del archivo en pos de evitar repetidos.');
